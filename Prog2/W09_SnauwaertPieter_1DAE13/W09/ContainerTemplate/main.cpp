...
void TestVector2fContainer()
{
	...
	Container<Vector2f> velocities{ Vector2f{650,98},Vector2f{589,685}, Vector2f{135,667} };
	// TODO: 8. Add some elements, enough to expand the container
	const int amountToAdd{ 4 };
	for (int i{}; i < amountToAdd; ++i)
	{
		Vector2f randVelocity{ float(rand() % 999),float(rand() % 999) };
		velocities.PushBack(randVelocity);
	}
