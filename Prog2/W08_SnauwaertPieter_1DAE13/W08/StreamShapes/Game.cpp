...
Color4f Game::ToColor(const std::string& colorStr) const
{
	...
	Color4f color;
	const int rgbaIndex{4};
	for (int i{}; i < rgbaIndex; ++i)
	{
		std::getline(ss, colorString, ',');
		switch (i)
		{
		case 0:
			color.r = std::stof(colorString);
			break;
		case 1:
			color.g = std::stof(colorString);
			break;
		case 2:
			color.b = std::stof(colorString);
			break;
		case 3:
			color.a = std::stof(colorString);
			break;
		}
	}
	return color;
