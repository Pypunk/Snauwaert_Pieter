...
void utils::DrawCircle(const Point2f& center, float rad, float lineWidth)
{
	DrawEllipse(center, rad, rad, lineWidth);
}

void utils::DrawCircle(const Circlef& circle, float lineWidth)
{
...
void utils::FillCircle(const Point2f& center, float rad)
{
	FillEllipse(center, rad, rad);
}

void utils::FillCircle(const Circlef& circle)
{
...
float utils::GetDistance(const Point2f& point1, const Point2f& point2)
{
	return sqrtf(powf(point2.x - point1.x, 2) + powf(point2.y - point1.y, 2));
}

float utils::GetDistance(float p1x, float p1y, float p2x, float p2y)
{
	return sqrtf(powf(p2x - p1x, 2) + powf(p2y - p1y, 2));
}
Point2f utils::GetMiddle(const Point2f& pointA, const Point2f& pointB)
{
	return Point2f{ (pointA.x + pointB.x) / 2.f, (pointA.y + pointB.y) / 2.f };
}

Point2f utils::GetMiddle(float p1x, float p1y, float p2x, float p2y)
{
	return Point2f{ (p1x + p2x) / 2.f, (p1y + p2y) / 2.f };
}

Point2f utils::GetMiddle(const Rectf& rect)
{
...
bool utils::Raycast(const Rectf& rect, const Point2f& rayP1, const Point2f& rayP2, HitInfo& hitInfo)
{
	return Raycast(GetTopVerticesFromRect(rect), rayP1, rayP2, hitInfo);
}
...
float utils::ConvertToDegrees(float radians)
{
	return radians * (180 / g_Pi);
}
Point2f utils::CreateCoordinatesFromRads(float radius, float radians, const Point2f& offset)
{
...
int utils::GetRowIndex(int index, int nrCols)
{
	return index / nrCols;
}
int utils::GetColIndex(int index, int nrCols)
{
	return index % nrCols;
}
std::vector<Point2f> utils::GetTopVerticesFromRect(const Rectf& rect)
{
	std::vector<Point2f> vector;
	vector.push_back(Point2f{ rect.left,rect.bottom + rect.height });
	vector.push_back(Point2f{ rect.left + rect.width,rect.bottom + rect.height });
	return vector;
}
